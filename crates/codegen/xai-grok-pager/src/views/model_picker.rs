...
impl ModelPicker {
    ...
    pub fn handle_event(&mut self, event: &Event) -> ModelPickerOutcome {
        ...
        match outcome {
            ...
            PickerOutcome::Changed | PickerOutcome::QueryChanged => ModelPickerOutcome::Changed,
...
mod tests {
    ...
    #[test]
    fn paste_refreshes_filter_and_sanitizes_line_breaks() {
        let mut picker = ModelPicker::new(&sample_models(), false);
        assert_eq!(
            picker.handle_event(&Event::Paste("plain\r\n".to_owned())),
            ModelPickerOutcome::Changed
        );
        assert_eq!(picker.picker.query(), "plain");
        assert_eq!(picker.filtered_indices, vec![1]);
    }
