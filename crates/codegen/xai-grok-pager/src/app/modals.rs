...
mod models_picker_live_validation_tests {
    ...
    use crate::views::model_picker::{ModelPicker, ModelPickerSelection};
    ...
    #[test]
    fn models_picker_paste_routes_to_search_without_touching_prompt() {
        let mut agent = make_agent();
        agent.prompt.set_text("hidden prompt");
        let models = live_models();
        agent.session.models = models.clone();
        agent.active_modal = Some(ActiveModal::ModelsPicker {
            picker: ModelPicker::new(&models, false),
        });
