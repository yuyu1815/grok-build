...
pub fn format(english: &str, values: &[(&str, String)]) -> String {
    ...
    while let Some(open) = rest.find('{') {
        ...
        if let Some((_, value)) = values.iter().find(|(candidate, _)| *candidate == name) {
            ...
            rendered.push_str(&rest[open..open + close + 2]);
        }
...
mod tests {
    ...
    #[test]
    fn format_replaces_only_template_placeholders_once() {
        assert_eq!(
            format(
                "{first} | {second} | {missing} | {first}",
                &[
                    ("first", "value with {second} and {{braces}}".to_string()),
                    (
                        "second",
                        r"C:\\work\\{first}\\file.rs: error[E0001] {unknown}".to_string(),
                    ),
                ],
            ),
            r"value with {second} and {{braces}} | C:\\work\\{first}\\file.rs: error[E0001] {unknown} | {missing} | value with {second} and {{braces}}"
        );
    }

    #[test]
    fn format_preserves_unknown_and_unclosed_placeholders() {
        assert_eq!(
            format(
                "known={known}; unknown={unknown}; literal={",
                &[("known", "ok".to_string())]
            ),
            "known=ok; unknown={unknown}; literal={"
        );
    }
